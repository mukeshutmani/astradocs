...
13. **Customer/Supplier-driven Document picker in Manual JE editor** (`JournalEntriesImproved.jsx`)
    - Backend: `GET /journalEntry/related-documents?customer_id=X` returns invoices via `documents → service → order.customer_id`; `?supplier_id=Y` returns costing/XO documents via `documents → service.supplier_id`. Both queries:
      ...
      - **Amount calculation (invoices)**: read `invoice.total_amount` and multiply by `resolveRate(exchange_rate, currency)` once. Invoices store `total_amount` consistently in source currency.
    ...
    - **Frontend label**: `JournalEntriesImproved.jsx` formats each dropdown option as `"<document_number> (<formatted-amount> PKR) (<source_currency>)"`. The second bracket is omitted for PKR docs — e.g., `KHIN00000150 (5,000.00 PKR)` for PKR, `KHIN00000078 (90,000.00 PKR) (AUD)` for foreign-currency docs. `searchLabel` remains the bare `document_number` so type-to-search still works against the doc number alone.

...
## Rules
1. **Idle timeout — configurable via `IDLE_TIMEOUT_MS` (currently 40 minutes).** If there is
   no user activity (mouse move, mouse down, key press, scroll, touch, click) for that long,
   the user is logged out.
2. **Absolute hard cap — 4 hours.** No login can last longer than 4 hours total, even for a
...
3. **Idle check:** a timer runs every 15 seconds and compares `now - lastActivity` against the
   idle threshold.
4. **Absolute cap:** the 4-hour limit is read directly from the login token's own `exp` claim
...
1. **Backend token:** issued with `expiresIn: "4h"` in `psback/controllers/auth.controller.js`.
...
2. **redux-persist-expire:** `psfront/src/store/index.js` expires the persisted `user`
   slice after 4 hours (`expireSeconds: 3600*4`), kept in sync with the token as a second layer.
...
- Short-lived backend token + silent refresh-on-activity. The current idle timeout logs the
  UI out, but the token itself stays valid on the server for its full life (now 4 hours). For

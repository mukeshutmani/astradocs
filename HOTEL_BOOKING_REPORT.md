...
## Output
- **Filter retention & inline preview (frontend)**: Selected filters are retained after generating (module-level `cachedFilter` in `HotelBookingReport.jsx`); returning to the page keeps prior selections, and a full page refresh resets to `DEFAULT_FILTER`. PDF is rendered inline below the filters (iframe → `{VITE_API_URL}/document/view/{report_number}`) with a **Preview** button (opens the full PDF in a new tab); no navigation away from the page. Excel still downloads and clears the inline PDF.
